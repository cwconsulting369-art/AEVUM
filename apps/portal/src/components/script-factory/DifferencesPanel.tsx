  ...
  if (!differences || differences.length === 0) {
    return (
      <div className="card-premium p-5 text-center text-sm text-ink-400">
        Keine strukturierten Differenzen vorhanden — Pipeline lieferte freien Output.
      </div>
    );
  }
    ...
    <div className="card-premium p-5">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="text-sm font-semibold text-white">
          {count ?? differences.length} Verbesserungen identifiziert
        </h3>
      </div>
        ...
        {differences.map((d, i) => (
          <li key={i} className="border-l-2 border-gold-400/40 pl-3 py-1">
            <div className="flex items-start justify-between gap-3 mb-1">
              <div className="text-sm font-medium text-white flex-1">{d.what || `Change ${i + 1}`}</div>
